...
为 le-bot-frontend 设计并实现一套完整的前端遥测埋点系统，涵盖页面浏览、点击事件、停留时长、用户留存、转化漏斗、行为路径等数据维度，支持批量上报/离线缓存/采样控制，后端自建分析看板，需做隐私合规处理。
产品概述
...
关键设计决策
为何自建而非用第三方 SDK：后端自建分析看板，数据格式需高度定制；涉及儿童隐私合规，数据不外传第三方
为何用 Vue Router 守卫而非逐页埋点：40+ 页面逐页改动量大、易遗漏、维护成本高；路由守卫一处实现，零业务侵入
为何 IndexedDB 而非 localStorage：埋点数据量大（单用户日积累可能数千条），localStorage 5MB 限制不够，IndexedDB 无容量压力
为何采样在客户端而非服务端：减少无效网络传输，核心页面全量、次要页面可采样，灵活可控
用户标识方案：userIdHash = SHA-256(accessToken + salt)，未登录用户使用 deviceFingerprint（浏览器指纹组合）
架构设计
系统架构图
传输层
引擎层 src/utils/telemetry/
业务层
批量/定时
上报失败/离线
online 事件补发
API: POST /telemetry/batch
Axios Instance 复用
TelemetryEngine 引擎
SamplingFilter 采样过滤
BufferQueue 缓冲队列
...
`src/utils/telemetry/engine.ts`

事件调度中心，接收各来源事件，驱动过滤与缓冲

SamplingFilter

`src/utils/telemetry/sampling.ts`

按事件/页面类型匹配采样率，决定是否丢弃

BufferQueue

`src/utils/telemetry/buffer.ts`
...
IndexedDB 持久化队列，管理离线重试与补发

TelemetryAPI

`src/utils/api/telemetry.ts`

批量上报 HTTP 接口，复用现有 axios

v-track 指令

`src/directives/track.ts`

Vue 指令，绑定 click 自动上报

useTracker

`src/composables/useTracker.ts`

Composable，支持页面内自定义事件上报

TelemetryStore

`src/stores/telemetry/index.ts`

Pinia store，管理 sessionId、设备指纹、设备 ID

Boot Plugin

`src/boot/telemetry.ts`

启动插件，初始化引擎 + 注册路由守卫 + 注册全局指令
...
任务列表
+ 新建
1. 定义遥测系统类型：TelemetryEventType、TelemetryEvent、TelemetryBatchRequest/Response 及相关枚举
2. 实现遥测核心工具层：隐私过滤(privacy.ts)、设备指纹(fingerprint.ts)、采样过滤(sampling.ts)、缓冲队列(buffer.ts)、离线队列(offline.ts)、API 客户端(telemetry.ts)
3. 实现 TelemetryEngine 引擎(engine.ts)：事件调度中心，统一管理事件入队、隐私过滤、采样判定、缓冲与离线降级
4. 创建 TelemetryStore Pinia store：管理 sessionId、deviceFingerprint、deviceId、sessionStart 等状态
5. 创建 v-track 指令和 useTracker composable：声明式点击埋点 + 命令式自定义事件上报
6. 创建 boot/telemetry.ts 启动插件：初始化引擎、注册 v-track 全局指令、注入 useTracker
7. 在 router/index.ts 添加 afterEach 路由守卫：自动采集页面 PV/Enter/Leave/Stayout 事件
8. 在 quasar.config.ts 的 boot 数组中注册 'telemetry' 插件
9. 使用 [subagent:code-explorer] 探索核心页面，为关键业务交互添加 v-track 和 useTracker 埋点（首页乐宝点击/设备切换/按住说话/添加设备流程/认证步骤/家庭组邀请/设置项点击等）
10. 编写 Telemetry API 接口规范文档，补充到 docs/api-specification.md 中，定义 POST /api/v1/telemetry/batch 的请求/响应格式和错误码